...
class VigenereCipheringMachine {
  ...
  encrypt(message, key) { //зашифровка
    ...
    for (let i = 0; i < NEW_MESSAGE.length; i++) { //отделение символов
    ...
    const FINNALY_ARR = Array(NEW_MESSAGE.length); //создания выходного массива с добавлением слотов
    const RESULT_ARR = []; //код на выходе зашифрованный
  ...
  decrypt(encryptedMessage, key) { //расшифровка
    ...
    const NEW_MESSAGE = encryptedMessage.toUpperCase();
    ...
    const MESSAGE_ARR = [...'' + encryptedMessage];

    const KEY_ARR = [...'' + key.repeat(encryptedMessage.length)];
    KEY_ARR.length = encryptedMessage.length; //ограничение длины
    ...
    if (this.valueStart === undefined || this.valueStart === true) {
      for (let i = 0; i < MESSAGE_ARR.length; i++) {
        let index = TABLE_OBJ[MESSAGE_ARR[i]] - TABLE_OBJ[KEY_ARR[i]];
        if (index < 0) index = TABLE_OBJ[MESSAGE_ARR[i]] + this.arr.length - TABLE_OBJ[KEY_ARR[i]];
        RESULT_ARR.push(this.arr[index]);
      }
